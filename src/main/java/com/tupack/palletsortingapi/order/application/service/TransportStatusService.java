...
public class TransportStatusService {
    ...
    @Transactional
    public GenericResponse updateTransportStatus(Long orderId, TransportStatus newStatus,
        Double latitude, Double longitude, String address, String notes) {
        ...
        if (order.getOrderStatus() != OrderStatus.IN_PROGRESS) {
            throw new BusinessException(String.format(
                "Cannot update transport status for order in status: %s. "
        ...
        if (currentStatus != null && !currentStatus.canTransitionTo(newStatus)) {
            throw new BusinessException(
                String.format("Invalid transport status transition from %s to %s", currentStatus,
                    newStatus), "INVALID_TRANSPORT_STATUS_TRANSITION");
        }

        // Create status update record
        String updatedBy = getCurrentUsername();
        TransportStatusUpdate statusUpdate =
            TransportStatusUpdate.builder().order(order).status(newStatus)
                .timestamp(LocalDateTime.now()).locationLatitude(latitude)
                .locationLongitude(longitude).locationAddress(address).notes(notes)
     ...
     */
    @Transactional
    public GenericResponse updateTransportStatusWithPhoto(Long orderId, TransportStatus newStatus,
        String photoUrl, Double latitude, Double longitude, String notes) {

        GenericResponse response =
            updateTransportStatus(orderId, newStatus, latitude, longitude, null, notes);

        // Update the latest status update with photo
        TransportStatusUpdate latestUpdate =
            transportStatusUpdateRepository.findFirstByOrder_IdOrderByTimestampDesc(orderId)
                .orElseThrow(() -> new BusinessException("Status update not found",
    ...
    public GenericResponse getTransportStatusHistory(Long orderId) {
        ...
        return GenericResponse.success(
            history.stream().map(transportStatusUpdateMapper::toDto).collect(Collectors.toList()));
    }
